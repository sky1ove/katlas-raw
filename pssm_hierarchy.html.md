...
This notebook is about **hierarchical clustering of kinases based on
their substrate specificity profiles** (Position-Specific Scoring
Matrices).

Here are what it covers:

1.  **Two datasets are being analyzed:**

    - **CDDM** (328 kinases) — likely from a cell-derived data method
    - **PSPA** (appears larger, ~395 kinases) — Positional Scanning
      Peptide Array data

2.  **The PSSM data** shows the probability of each amino acid appearing
    at positions -20 to +20 around a phosphorylation site (the columns
    like `-20P`, `-20G`, `20pS`, `20pY`, etc.)

3.  **The clustering** groups kinases with similar substrate preferences
    together, and the dendrograms visualize these relationships

4.  **The labels** show each kinase’s preferred phospho-acceptor motif
    (like `y*` for tyrosine kinases) and sample counts

The two dendrogram images show the hierarchical relationships between
kinases based on what peptide sequences they prefer to phosphorylate.
...
``` python
plot_dendrogram(Z,dense=5,labels=labels) 
save_svg('out/CDDM_hierarchy.svg')
```

![](04_PSSM_hierarchy_files/figure-commonmark/cell-10-output-1.png)
...
``` python
plot_dendrogram(Z2,dense=5,labels=labels2) 
save_svg('out/PSPA_hierarchy.svg')
```

![](04_PSSM_hierarchy_files/figure-commonmark/cell-16-output-1.png)
